#!/usr/bin/env python3
"""
TopDesk API - FortiSwitch Device Export

This script retrieves all Fortinet FortiSwitch devices from TopDesk
...
def filter_fortiswitch_devices(assets: List[Dict]) -> List[Dict]:
    """
    Filter assets to get only FortiSwitch devices.
    
    Uses OR logic to match ANY of these conditions:
    - Summary contains "FortiSwitch"
    - Summary contains "Fortinet" with common FortiSwitch patterns
    - Common FortiSwitch model numbers
    ...
    for asset in assets:
        ...
        if not ('switch' in type_name or 'swi' in type_name):
            continue

        is_fortiswitch = any([
            'fortiswitch' in summary,
            'fortinet' in summary and 'fs-' in summary,
            'fortinet' in summary and 'fs' in summary.split(),
            any(model in summary for model in [
                '108e', '108f', '124e', '124f', '148e', '148f',
                '224e', '224f', '248e', '248f', '424e', '448e',
                '1024e', '1048e', '3032e', 'fs-108', 'fs-124',
                'fs-148', 'fs-224', 'fs-248', 'fs-424', 'fs-448',
                'fs-1024', 'fs-1048', 'fs-3032'
            ])
        ])

        if is_fortiswitch:
            fortiswitch_devices.append(asset)

    return fortiswitch_devices
...
def extract_serial_from_text(text: str) -> Optional[str]:
    ...
    matches = re.findall(r'\b(FS[A-Z0-9]{6,}|S\d{3}[A-Z]{2}[A-Z0-9]{6,})\b', text, re.IGNORECASE)
    if matches:
        return matches[0]
...
def main():
    ...
    print("="*70)
    print("TopDesk API - FortiSwitch Device Export")
    print("="*70)
    print()
    ...
    print("Step 1: Retrieving all Switch [SWI] assets...")
    all_switches = api.get_assets_by_template('Switch [SWI]')
    print(f"Retrieved {len(all_switches)} switch assets")
    print()

    print("Step 2: Filtering for Fortinet FortiSwitch devices...")
    fortiswitch_devices = filter_fortiswitch_devices(all_switches)
    print(f"Found {len(fortiswitch_devices)} Fortinet FortiSwitch devices")
    print()

    print("Step 3: Enriching with support contracts...")
    enriched_devices = enrich_with_contracts(api, fortiswitch_devices)
    print()
    ...
    filename = f"td_fortiswitch_devices_{timestamp}.csv"
    ...
    print(f"Total devices: {len(fortiswitch_devices)}")
